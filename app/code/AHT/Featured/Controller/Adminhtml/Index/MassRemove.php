 ...
 */
class MassRemove extends \Magento\Catalog\Controller\Adminhtml\Product implements HttpPostActionInterface
{
    /**
     ...
     */
    public function _validateMassRemove(array $productIds, $featured)
    {
        if ($featured == \AHT\Featured\Model\Product\Attribute\Source\Featured::IS_FEATURED_ENABLED) {
    ...
    public function execute()
    {
        ...
        $featured = (int) $this->getRequest()->getParam('is_featured');
        ...
        try {
            $this->_validateMassRemove($productIds, $featured);
            $this->_objectManager->get(\Magento\Catalog\Model\Product\Action::class)
                ->updateAttributes($productIds, ['is_featured' => $featured], (int) $storeId);
        ...
        return $resultRedirect->setPath('featured/*/', ['store' => $requestStoreId]);
    }
}
